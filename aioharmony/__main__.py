...
async def get_client(ip_address, protocol, show_responses) -> Optional[HarmonyAPI]:
    ...
    try:
        if await client.connect():
            print(
                "Connected to HUB {} ({}) with firmware version {} and HUB ID {} using protocol {}".format(
                    client.name,
                    ip_address,
                    client.fw_version,
                    client.hub_id,
...
async def just_listen(client, args):
    ...
    print(
        "Starting to listen on HUB {} with firmware version {}".format(
...
async def send_command(client, args):
    ...
    if result_list:
        for result in result_list:
            print(
                "HUB: {} Sending of command {} to device {} failed with code {}: "
                "{}".format(
                    client.name,
...
async def run():
    ...
    show_detailed_config_parser = subparsers.add_parser(
        ...
        help="Print the detailed Harmony device" " configuration.",
    )
    ...
    command_parser.add_argument(
        ...
        type=float,
        default=0,
        help='Number of seconds to "hold" before releasing. Defaults to 0.4' "seconds",
    )
